    ...
    , ui(new Ui::Widget)
{
    ...
    QObject::connect(this,&Widget::initStudentDatabase,[=](){ //初始化教师数据库
        this->m_loginContorller->initStudentDatabase();
    });
    QObject::connect(this,&Widget::initStudentTable,[=](){//初始化教师数据库表
        this->m_loginContorller->initStudentTable();
    });
    QObject::connect(this->ui->pushButton_2,&QPushButton::clicked,this,&Widget::initRegisterDlg);
    emit this->initStudentDatabase(); //注意先绑定信号槽，再进行发出信号
    emit this->initStudentTable();
}
