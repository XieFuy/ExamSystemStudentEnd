...
//    CDBHelper* dbHelper = CDBHelper::getInstance();
    ...
    sprintf(buf,"select count(*) from `Student` where studentId = '%s';",pAcount);
    sql = buf;
    ...
    memset(buf,'\0',sizeof(char) * 1024);
    sprintf(buf,"insert into `Student`(`studentId`,`password`,`role`) values('%s','%s','student');",pAcount,pPassword);
    sql = buf;
    bool ret =  dbHelper->sqlExcute(sql,"ExamSystem");
    //在完成注册时候还要进行一个对学生信息表的插入
    memset(buf,'\0',sizeof(char) * 1024);
    sprintf(buf,"insert into `StudentInfo`(`studentId`) values('%s');",pAcount);
    sql = buf;
